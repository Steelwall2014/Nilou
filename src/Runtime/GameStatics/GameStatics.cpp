...
namespace nilou {
	...
	std::vector<std::shared_ptr<UStaticMesh>> GameStatics::ParseToStaticMeshes(tinygltf::Model &model)
    {
        ...
        for (auto &gltf_mesh : model.meshes)
        {
            ...
            for (int prim_index = 0; prim_index < gltf_mesh.primitives.size(); prim_index++)
            {
                ...
                FStaticMeshSection &Section = Resource->Sections.emplace_back();
    ...
    std::vector<std::shared_ptr<FMaterial>> GameStatics::ParseToMaterials(tinygltf::Model &model)
    {
        ...
        for (tinygltf::Material &gltf_material : model.materials)
        {
            ...
            auto AccessTextures = [
                ...
                if (index != -1)
                {
                    Material->SetParameterValue(sampler_name, Textures[index].get());
