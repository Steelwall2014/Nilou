...
public:
    ...
    void SetDefine(std::string a, bool value) { defines[a] = value; }
    void SetDefine(std::string a, int32 value) { defines[a] = value; }
};
...
class FMyDimensionA : SHADER_PERMUTATION_BOOL("A");
class FMyDimensionB : SHADER_PERMUTATION_INT("B", 5);
class FMyDimensionC : SHADER_PERMUTATION_BOOL("C");
class FMyDimensionD : SHADER_PERMUTATION_INT("B", 5);
using FMyPermutationDomain = TShaderPermutationDomain<FMyDimensionA, FMyDimensionB, FMyDimensionC, FMyDimensionD>;
// using FMyPermutationDomain2 = TShaderPermutationDomain<FMyPermutationDomain, FMyDimensionC>;

int main()
{
    FMyPermutationDomain PermutationVector;
    PermutationVector.Set<FMyDimensionA>(true);
    PermutationVector.Set<FMyDimensionB>(2);
    PermutationVector.Set<FMyDimensionC>(true);

    auto id = PermutationVector.ToDimensionValueId();
    FMyPermutationDomain v(id);
    bool a = v.Get<FMyDimensionA>();
    int b = v.Get<FMyDimensionB>();
    bool c = v.Get<FMyDimensionC>();

    std::vector<int32> all_ids = FMyPermutationDomain::GetAllPermutationIds();
    return 0;
}