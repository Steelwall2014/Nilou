...
namespace nilou {
    ...
    void UActorComponent::RegisterComponentWithWorld(UWorld *InWorld)
    {
        if (IsRegistered())
        {
            NILOU_LOG(Display, "RegisterComponentWithWorld ({}) Already registered. Aborting.", GetClass()->GetName());
            return;
        }
        if (InWorld == nullptr)
        {
        ...
        if(MyOwner)
        {
            if(InWorld != MyOwner->GetWorld())
            {
                ...
                NILOU_LOG(Display, "RegisterComponentWithWorld: ({}) Specifying a world, but an Owner Actor found, and InWorld is not GetOwner()->GetWorld()", GetClass()->GetName());
            }
        }
        if (!bHasBeenCreated)
        {
