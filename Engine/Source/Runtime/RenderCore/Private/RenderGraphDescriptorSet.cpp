...
void RDGDescriptorSet::SetCombinedTextureSampler(int32 BindingIndex, RDGTextureView* Texture, RHISamplerState* SamplerState)
{
	if (auto Binding = GetBindingByIndex(BindingIndex))
	{
		SetSamplerInternal(*Binding, Texture, SamplerState);
...
void RDGDescriptorSet::SetStorageBuffer(int32 BindingIndex, RDGBuffer* Buffer)
{
	if (auto Binding = GetBindingByIndex(BindingIndex))
	{
		SetStorageBufferInternal(*Binding, Buffer);
...
void RDGDescriptorSet::SetStorageImage(int32 BindingIndex, RDGTextureView* Image)
{
	if (auto Binding = GetBindingByIndex(BindingIndex))
	{
		SetStorageImageInternal(*Binding, Image);
...
void RDGDescriptorSet::SetSamplerState(int32 BindingIndex, RHISamplerState* SamplerState)
{
	if (auto Binding = GetBindingByIndex(BindingIndex))
	{
		SetSamplerStateInternal(*Binding, SamplerState);
	}
}
...
void RDGDescriptorSet::SetSamplerInternal(const RHIDescriptorSetLayoutBinding& Binding, RDGTextureView* Texture, RHISamplerState* SamplerState)
{
	Ncheck(Binding.DescriptorType == EDescriptorType::CombinedImageSampler);
	DescriptorImageInfo ImageInfo;
	ImageInfo.SamplerState = SamplerState ? SamplerState : TStaticSamplerState<SF_Trilinear>::GetRHI();
...
void RDGDescriptorSet::SetStorageBufferInternal(const RHIDescriptorSetLayoutBinding& Binding, RDGBuffer* Buffer)
{
...
void RDGDescriptorSet::SetStorageImageInternal(const RHIDescriptorSetLayoutBinding& Binding, RDGTextureView* Image)
{
...
void RDGDescriptorSet::SetSamplerStateInternal(const RHIDescriptorSetLayoutBinding& Binding, RHISamplerState* SamplerState)
{
	Ncheck(Binding.DescriptorType == EDescriptorType::Sampler);
	DescriptorImageInfo ImageInfo;
	ImageInfo.SamplerState = SamplerState ? SamplerState : TStaticSamplerState<SF_Trilinear>::GetRHI();
	WriteDescriptorSet WriteDescriptor;
	WriteDescriptor.DstBinding = Binding.BindingIndex;
