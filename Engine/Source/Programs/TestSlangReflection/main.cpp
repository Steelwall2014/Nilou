...
static const char* kSourceFileNames[] = {
    "../../../../Engine/Source/Programs/TestSlangReflection/raster-simple.slang",
    "../../../../Engine/Source/Programs/TestSlangReflection/compute-simple.slang",
};
...
#define SCOPED_OBJECT() ScopedObject scopedObject##__COUNTER__(this)
    ...
    Result compileAndReflectProgram(slang::ISession* session, const char* sourceFileName)
    {
        ...
        module = session->loadModule(sourceFilePath.c_str(), diagnostics.writeRef());
        diagnoseIfNeeded(diagnostics);
        if (!module)
            return SLANG_FAIL;

        vector<ComPtr<slang::IComponentType>> componentsToLink;
        ...
        result = session->createCompositeComponentType(
            ...
            (SlangInt)componentsToLink.size(),
            composed.writeRef(),
...
struct ExampleProgram
{
    ...
    {
        // parseOption removed - not needed for this example

        ComPtr<slang::IGlobalSession> globalSession;
        SLANG_RETURN_ON_FAIL(slang::createGlobalSession(globalSession.writeRef()));

        vector<slang::TargetDesc> targetDescs;
